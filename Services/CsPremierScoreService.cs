...
internal sealed class CsPremierScoreService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            CsAccountProfile? profile = null;
            try
            {
                var profileMessage = await profileTask;
                profile = DecodePlayersProfile(accountId, profileMessage.Payload);
            }
            catch (Exception)
            {
                // Ignore profile errors, we mainly need the helloData for cooldown
            }
            ...
            return new CsPremierScoreResult(
                ...
                accountId,
                premier,
                profile?.Rankings ?? Array.Empty<CsRankingInfo>(),
